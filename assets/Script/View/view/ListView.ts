...
export default class ListView extends cc.Component {
    ...
    private adapter: AbsAdapter<cc.Component> = null;
    ...
    public async setAdapter(adapter: AbsAdapter<cc.Component>) {
...
// 数据绑定的辅助适配器
export abstract class AbsAdapter<T extends cc.Component> {
    private readonly itemComponent: { prototype: T } = null
    private dataSet: any[] = [];

    public constructor(clz: { prototype: T }) {
        this.itemComponent = clz;
    }
    ...
    public _getView(item: cc.Node, posIndex: number): cc.Node {
        let itemComp = item.getComponent<T>(this.itemComponent);
        if (itemComp) {
            this.updateView(itemComp, posIndex);
        } else {
            console.warn("item 不包含组件:", this.itemComponent);
        }
        return item;
    }

    public abstract updateView(item: T, posIndex: number);
}